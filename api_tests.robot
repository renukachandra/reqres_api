...
*** Test Cases ***
CREATE User creation should be successful
    &{data}=    Create dictionary  name=morpheus  job=leader
    ${response}=    POST  ${users_URL}  json=${data}  expected_status=201

UPDATE User update should be successful
    &{data}=    Create dictionary  name=morpheus  job=leader
    ${response}=    PUT  ${users_URL}/2  json=${data}  expected_status=200
